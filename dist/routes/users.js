...
router.get('/branch', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
...
router.put('/:id/password', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        const { id } = req.params;
...
router.delete('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (targetUser._count.processedRequests > 0) {
            await database_1.default.userRequest.updateMany({
        ...
        if (targetUser._count.ticketHistory > 0) {
            await database_1.default.ticketHistory.deleteMany({
                where: { changedBy: id }
            });
        }
        ...
        if (targetUser._count.uploadedAttachments > 0) {
            ...
            const alternativeUser = await database_1.default.user.findFirst({
                where: {
                    id: { not: id },
                    role: 'ADMIN',
                    isActive: true
            ...
            if (alternativeUser) {
                ...
                await database_1.default.attachment.updateMany({
                    where: {
                        uploadedById: id,
                ...
                // Los adjuntos asociados a comentarios se eliminarán cuando eliminemos los comentarios
                await database_1.default.attachment.deleteMany({
                    where: {
                        uploadedById: id,
                        commentId: null // Solo adjuntos directamente en tickets
                    }
        ...
        if (targetUser._count.comments > 0) {
            await database_1.default.comment.deleteMany({
                where: { userId: id }
            });
        }
