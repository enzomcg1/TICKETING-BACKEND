...
router.post('/register', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existingUser = await database_1.default.user.findUnique({ where: { email: email.trim() } });
        if (existingUser) {
        ...
        loggerService_1.loggerService.info(`Usuario creado: ${user.name} (${user.email}) con rol ${user.role}`, 'USER', {
            userId: req.user?.id,
            metadata: {
...
router.post('/login', security_2.loginRateLimit, async (req, res) => {
    try {
        ...
        (0, security_2.clearLoginRateLimit)(req);
        res.json({
