...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (search && searchType) {
            ...
            const searchTypeParam = (searchType || 'all').toLowerCase();
        ...
        if (userId) {
            searchConditions.push({ requestedById: userId }, { assignedToId: userId });
        }
        if (ticketId) {
        ...
        if ((0, auth_1.canViewAllTickets)(user)) {
        ...
        if (isExactNumberSearch && exactTicketNumber !== null) {
            ...
            if (Object.keys(permissionWhere).length > 0) {
                const andConditions = [{ ticketNumber: exactTicketNumber }];
...
router.get('/export', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const fullUser = await database_1.default.user.findUnique({
            where: { id: user.id },
        ...
        const getLastStatusChange = (ticket) => {
        ...
        tickets.forEach((ticket, index) => {
            ...
            const row = worksheet.addRow([
                ...
                ticket.title,
                ticket.description || '',
                statusService_1.statusLabels[ticket.status] || ticket.status,
                ticket.priority,
                ticket.category || '',
        ...
        worksheet.columns.forEach((column, index) => {
            if (index === 0)
                column.width = 12; // Nº Ticket
            else if (index === 1)
                column.width = 30; // Título
            else if (index === 2)
                column.width = 40; // Descripción
            else if (index === 3)
                column.width = 15; // Estado
            else if (index === 4)
                column.width = 12; // Prioridad
            else if (index === 5)
                column.width = 15; // Categoría
            else if (index === 6)
                column.width = 20; // Solicitante
            else if (index === 7)
                column.width = 25; // Email Solicitante
            else if (index === 8)
                column.width = 20; // Técnico
            else if (index === 9)
                column.width = 25; // Email Técnico
            else if (index === 10)
                column.width = 20; // Sucursal
            else if (index === 11)
                column.width = 15; // Código Sucursal
            else if (index === 12)
                column.width = 15; // Ciudad
            else if (index === 13)
                column.width = 20; // Departamento
            else if (index === 14)
                column.width = 15; // Código Departamento
            else if (index === 15)
                column.width = 20; // Fecha Creación
            else if (index === 16)
                column.width = 20; // Fecha Actualización
            else if (index === 17)
                column.width = 20; // Fecha Resolución
            else if (index === 18)
                column.width = 40; // Resolución
            else if (index === 19)
                column.width = 20; // Último Cambio
            else if (index === 20)
                column.width = 20; // Agente Cambio
            else
                column.width = 15;
        });
        ...
        await loggerService_1.loggerService.info(`Exportación de tickets a Excel realizada`, 'TICKET', {
            userId: user.id,
            metadata: {
...
router.get('/export-pdf', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        // Solo ADMIN, SUPERVISOR, AUDITOR y TECHNICIAN pueden exportar
        // USER no puede exportar
        ...
        const writeWrappedText = (text, fontSize = 10, color = '#000000') => {
        ...
        const drawLine = (y, color = borderGray, width = 1) => {
        ...
        const drawInfoBox = (label, value, x, y, width, height = 20) => {
            // Fondo gris claro
        ...
        if (status || priority || branchId || departmentId || startDate || endDate) {
            ...
            if (status) {
                drawInfoBox('Estado', statusService_1.statusLabels[status] || status, filterX, filterY, filterBoxWidth);
            ...
            if (priority) {
                drawInfoBox('Prioridad', priority, filterX, filterY, filterBoxWidth);
                filterX += filterBoxWidth + 10;
                if (filterX + filterBoxWidth > pageWidth + 40) {
                    filterX = 40;
                    filterY += 25;
            ...
            if (startDate) {
                drawInfoBox('Fecha Inicio', startDate, filterX, filterY, filterBoxWidth);
                filterX += filterBoxWidth + 10;
                if (filterX + filterBoxWidth > pageWidth + 40) {
                    filterX = 40;
                    filterY += 25;
            ...
            if (endDate) {
                drawInfoBox('Fecha Fin', endDate, filterX, filterY, filterBoxWidth);
                filterX += filterBoxWidth + 10;
                if (filterX + filterBoxWidth > pageWidth + 40) {
                    filterX = 40;
                    filterY += 25;
        ...
        tickets.forEach((ticket, index) => {
            ...
            drawInfoBox('Estado', statusService_1.statusLabels[ticket.status] || ticket.status, 40, yPosition, infoBoxWidth, infoBoxHeight);
        ...
        loggerService_1.loggerService.info(`Exportación de tickets a PDF realizada`, 'TICKET', {
            userId: user.id,
            metadata: {
                filters: { status, priority, branchId, departmentId, startDate, endDate },
                totalTickets: tickets.length
...
router.get('/stats', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const [total, byStatus, byPriority, byBranch, byDepartment, recentTickets] = await Promise.all([
        ...
        res.json({
            ...
            byStatus: byStatus.map(item => ({
                ...
                label: statusService_1.statusLabels[item.status] || item.status
            })),
...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const ticketNumber = await (0, ticketNumberGenerator_1.generateUniqueTicketNumber)();
        ...
        await database_1.default.ticketHistory.create({
            data: {
                ticketId: ticket.id,
                action: 'CREATED',
                changedBy: user.id,
        ...
        const fullTicket = await database_1.default.ticket.findUnique({
            where: { id: ticket.id },
            include: {
                requestedBy: {
                    select: { id: true, name: true, email: true }
        ...
        if (fullTicket) {
            (0, notificationService_1.notifyTicketCreated)(fullTicket).catch(err => {
...
router.put('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (status && status !== ticket.status) {
            ...
            const statusCheck = (0, statusService_1.canChangeStatus)(user, ticket.status, status, {
        ...
        if (historyEntries.length > 0) {
            await database_1.default.ticketHistory.createMany({
                data: historyEntries
            });
        }
        ...
        if ((status && status !== ticket.status) || (assignedToId !== undefined && assignedToId !== ticket.assignedToId && finalStatus === 'ASSIGNED')) {
            ...
            (0, notificationService_1.notifyTicketStatusChange)({
...
router.post('/:id/comments', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const hasPermission = (0, auth_1.canAddComment)(user, ticket);
        if (!hasPermission) {
        ...
        const commentUser = await database_1.default.user.findUnique({
            where: { id: user.id },
            select: { name: true, email: true }
        });
        // Notificar a los involucrados sobre el nuevo comentario
        (0, notificationService_1.notifyCommentAdded)(req.params.id, comment.id, content, {
            id: user.id,
...
router.delete('/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await loggerService_1.loggerService.info(`Ticket #${ticket.ticketNumber} eliminado por ${fullUser.name} (${fullUser.email})`, 'TICKET', {
            userId: user.id,
            ticketId: ticket.id,
            metadata: {
        ...
        await database_1.default.ticket.delete({
            where: { id: req.params.id }
        });
...
router.get('/:id/available-statuses', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        const ticket = await database_1.default.ticket.findUnique({
            where: { id: req.params.id },
        ...
        const availableStatuses = (0, statusService_1.getAvailableStatusTransitions)(user, ticket.status, {
            assignedToId: ticket.assignedToId,
            requestedById: ticket.requestedById,
            requestedByRole: ticket.requestedBy?.role,
        });
        res.json({
            ...
            availableStatuses: availableStatuses.map((status) => ({
                value: status,
                label: statusService_1.statusLabels[status],
            })),
        });
    }
    catch (error) {
